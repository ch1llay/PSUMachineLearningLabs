...
### Для гитариста
Чистый звук на нэке = мало пересечений (плавная волна). Хай-гейн на бридж-пикапе = много пересечений (дисторшн «обрезает» волну, создавая кучу крутых фронтов и мелких осцилляций). Если играешь palm mute — ZCR будет **ниже**, чем при открытых струнах на том же аккорде, потому что демпфирование подавляет высокочастотные обертоны — сигнал становится «тупым» и низкочастотным, а значит, реже пересекает ноль.
